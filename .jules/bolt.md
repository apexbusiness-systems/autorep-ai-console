...
## 2026-05-12 - Date-dependent test flakiness
**Learning:** Found a flaky test in `intelligence-maintenance-compliance.test.ts` where `scoreLead` calculations relied on real-time `Date.now()`, which broke when run past the hardcoded data's target date.
**Action:** Always wrap time-sensitive unit tests with `vi.spyOn(Date, 'now').mockReturnValue(...)` or `vi.useFakeTimers()` to ensure deterministic test execution regardless of when the suite is run. Ensure that the mock is properly restored (e.g. in `afterEach()`) to prevent leakage into other tests.
