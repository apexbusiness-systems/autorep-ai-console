...
## 2026-05-12 - Replacing O(N*M) normalized state lookup with a Map
**Learning:** Found that `useLeads()` derived `leadConversations` and `leadMessages` by continuously iterating over large base arrays and cross-referencing values inside `.map()` loops. This created O(N*M) performance bottlenecks with repeated filtering on every component re-render when the global state updated.
**Action:** When extracting grouped/related arrays for objects in a large loop from a normalized state dictionary, pre-group the entities into a `Map` to perform O(1) lookups inside the loop rather than repeatedly `.filter()`-ing across the entire dataset. Also ensure downstream functions receiving these arrays (like `scoreLead`) do not repeat the O(N*M) checks.
