...
## 2026-05-18 - Single-Pass Array Operations (Filter/Reduce/Length)
**Learning:** Found that `calculateQuoteTotals` and `suggestDynamicPrice` in `src/services/pricingService.ts` were chaining multiple `.filter()` and `.reduce()` operations or using `.filter().length` on arrays like `input.packages` and `input.vehicles`. This caused redundant O(N) traversals and intermediate array allocations.
**Action:** When computing multiple aggregates or counts from an array, replace chained array methods with a single-pass `for...of` loop to simultaneously compute the results. This eliminates redundant O(N) traversals and intermediate memory allocations.
