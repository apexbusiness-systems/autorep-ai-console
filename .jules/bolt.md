...
## 2026-07-15 - Lexicographical Date String Sorting
**Learning:** Discovered that parsing ISO 8601 date strings into Date objects and extracting the timestamp via `.getTime()` inside array sorting methods causes significant O(N log N) overhead and excessive memory allocation.
**Action:** When sorting arrays by ISO 8601 date strings, rely on standard lexicographical string comparison (e.g., `a > b ? -1 : 1`) to avoid expensive Date parsing and allocation.
