...
export function calculateQuoteTotals(input: QuoteCalculationInput): QuoteCalculationResult {
  ...
  // ⚡ Bolt Performance Optimization: Single-pass package calculation
  // Replaced chained .filter().reduce() with a single-pass loop to prevent redundant O(N) traversals and allocations
