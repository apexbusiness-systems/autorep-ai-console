...
class MarketCheckService {
  ...
  private getDemoListings(filters: InventorySearchFilters): InventoryListing[] {
    ...
    // Replaced multiple chained .filter() operations with a single pass
    // Expected impact: Prevents redundant O(N) traversals and intermediate array allocations.

    // Pre-calculate loop-invariant variables outside the filter
    const searchMake = filters.make?.toLowerCase();
    const searchModel = filters.model?.toLowerCase();
    const searchBodyType = filters.bodyType?.toLowerCase();
    ...
    return listings.filter(l => {
      ...
      if (searchBodyType && l.body.toLowerCase() !== searchBodyType) return false;
