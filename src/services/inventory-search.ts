...
class MarketCheckService {
  ...
  private getDemoListings(filters: InventorySearchFilters): InventoryListing[] {
    ...
    // Replaced multiple chained .filter() operations with a single pass that combines all condition checks.
    // Also extracted invariant calculations (e.g. toLowerCase()) outside the loop.
    // Expected impact: Eliminates redundant O(N) array traversals and intermediate memory allocations.
